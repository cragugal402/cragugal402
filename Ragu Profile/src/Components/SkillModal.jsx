...
const MobileSkillModal = styled(motion.div)`
...
const StaticChallengeCard = (props) => {
			...
			<motion.div
				variants={{
					...
					visible: {
						...
						boxShadow: `0px 0px ${(props.level > 2 ? props.level : 0) * 5}px ${
							props.level > 2 ? 1 : 0
						}px ${LightenColor(props.color, props.level * 10)}`,
						transition: {
							delay: 1,
...
const getChallenges = (challenges, skill) =>
	challenges.filter((challenge) => challenge.skills.indexOf(skill) !== -1);

const SkillModalContent = () => {
	const skillsContext = useContext(Experience);

	const [lastSkill, setLastSkill] = useState(skillsContext.selectedSkill);
			...
			<DescriptionContainer>
				...
				<LevelIndicator
					visible={
						...
						skillsContext.selectedSkill.key === lastSkill.key
					}
...
export const SkillModalComponent = (props) => {
	const skillsContext = useContext(Experience);
	...
	return (
		<AnimatePresence>
			<MediaQuery key={"mobile"} maxDeviceWidth={1224}>
				<MobileSkillModalContainer
					...
					initial={"hidden"}
					animate={
						skillsContext.selectedSkill !== null && props.inView
							? "visible"
							: "hidden"
					}>
					<MobileSkillModal
			...
			<MediaQuery key={"desktop"} minDeviceWidth={1224}>
				<SkillModalContainer
					variants={{
						hidden: {
							x: "-100vw",
